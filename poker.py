...
def game():
    if request.method == "POST":
        ...
        return render_template(
            "game2.html",
            player=table.humanPlayer,
            table=table,
            actionName=playerToAct.name,
            dealerName=table.currentPlayers[table.dealerPosition].name)
    # no changes, just general reloading page, no back-end action taken
    playerToAct = table.currentPlayers[
        table.actionPosition] if table.currentPlayers else None
    return render_template(
        "game2.html",
        player=table.humanPlayer,
        table=table,
...
def processPlayer():
    table.processPlayerAction(table.humanPlayer, request.form['action'],
                              int(request.form['bet_val']))
    playerToAct = table.currentPlayers[
        table.actionPosition] if table.currentPlayers else None
    return render_template(
        "game2.html",
