...
class PokerTestCase(unittest.TestCase):
    ...
    def test_incrementPosition(self):
        ...
        pos = 3
        # should rollover to beginning index
        pos = self.table.incrementPosition(pos, 4)
    ...
    def test_handStartConditions(self):
        ...
        for player in self.table.allPlayers:
            self.assertEqual(player.currentBet, 0,
                             "player bet not set to 0 before hand starts")
        self.assertEqual(self.table.pot, 0)
    ...
    def test_processPlayerAction(self):
        ...
        while self.table.actionPosition != self.table.bigBlindPosition:
            self.table.processPlayerAction(
                self.table.currentPlayers[self.table.actionPosition], "call")
        self.table.processPlayerAction(
            self.table.currentPlayers[self.table.actionPosition], "check")
        self.table.endBettingRound()
        self.assertEqual(self.table.pot, 10)
        self.assertEqual(self.table.actionPosition,
                         self.table.smallBlindPosition)
    ...
    def test_fullHand(self):
        ...
        lastActionPosition = self.table.actionPosition
        self.table.processPlayerAction(
            self.table.currentPlayers[self.table.actionPosition], "raise", 10)
